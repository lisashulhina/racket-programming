...
(define (rotate-polygon myPolygon colorNum)
  (cond
    [ (< colorNum 0.999)
      ...
      (send myPolygon translate -7.5 13) 
      (send dc draw-path myPolygon)
      (rotate-polygon myPolygon (+ colorNum 0.008325)) ; we will have 120 figures, sinwe we rotate by 3 degree 360/3=120. 0.999/120=0.008325 for color change
  ]
 )
)

;call recursive function with 0 for color start as a default
(define (drawToSceen myPolygon)
  (rotate-polygon myPolygon 0)
)


(drawToSceen myPolygon)
(display "\nRotated myPolygon: \n")
myTarget
